          ...
          {orders.map((order) => (
            <Card key={order.id} className="animate-fade-in hover:shadow-xl transition-shadow">
              ...
              <div className="pt-3 border-t flex justify-between items-center mb-4">
                ...
                <span className="font-bold text-xl text-green-600">{formatCurrency(order.total_amount)}</span>
              </div>
              ...
              <div className="space-y-2">
                {order.status === 'pending' && (
                  <Button onClick={() => updateOrderStatus(order.id, 'preparing')} variant="secondary" size="sm" className="w-full">
                    🍳 Start Preparing
                  </Button>
                )}
                {order.status === 'preparing' && (
                  <Button onClick={() => updateOrderStatus(order.id, 'ready')} variant="success" size="sm" className="w-full">
                    ✅ Mark Ready
                  </Button>
                )}
                {order.status === 'ready' && (
                  <Button onClick={() => updateOrderStatus(order.id, 'served')} variant="primary" size="sm" className="w-full">
                    🍽️ Mark Served
                  </Button>
                )}
              </div>
